...
win = Window("Solar System Explorer", size=(WIDTH, HEIGHT))
rend = Renderer(win)
...
bodies_info = [
    ...
    {"n": "JUPITER", "p": 400, "r": 55, "orb": 1200, "s": 0.007, "c": (220, 180, 140),
     "moons": [{"n": "Io", "p": 30, "r": 3, "orb": 80, "s": 0.08, "c": (255, 255, 100)}]},
    {"n": "SATURN", "p": 350, "r": 45, "orb": 1600, "s": 0.005, "c": (210, 200, 150), "rings": True},
]

planets = []
for b in bodies_info:
    t_surf = font.render(b["n"], True, (255, 255, 255))  # Белый текст для читаемости
...
while True:
    ...
    cur_acc = ACCEL * 12.0 if warp else ACCEL
    ...
    shake_x = random.uniform(-1, 1) * (speed * 0.005) if warp else 0
    shake_y = random.uniform(-1, 1) * (speed * 0.005) if warp else 0
    ...
    def project(tx, ty, tz):
        ...
        f = fov / rz
        return (int(rx * f + WIDTH // 2 + shake_x), int(ry * f + HEIGHT // 2 + shake_y), rz, f)
    ...
    for p in planets:
        ...
        # Данные для HUD (берём центр планеты)
        dist_to_cam = math.sqrt((bx - cam_x) ** 2 + (0 - cam_y) ** 2 + (bz - cam_z) ** 2)
        if dist_to_cam < 2500:
            proj_c = project(bx - cam_x, -cam_y, bz - cam_z)
            if proj_c: hud_elements.append((p["t_tex"], proj_c, p["t_size"]))
    ...
    # Отрисовка названий (СВЕРХУ ПЛАНЕТ)
    for tex, proj, size in hud_elements:
        # Центрируем по X и поднимаем вверх на 50-60 пикселей
        hud_rect = pygame.Rect(proj[0] - size[0] // 2, proj[1] - 60, size[0], size[1])
        rend.blit(tex, hud_rect)
