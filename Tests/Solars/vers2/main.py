...
class SpaceEngine:
    ...
    def render(self):
        self.rend.draw_color = (2, 2, 15, 255); self.rend.clear()
        ...
        for p in self.planets:
            ...
            for obj in [p] + p.moons:
                dist = math.dist(obj.pos, self.cam); is_t = (obj.name == self.target.name)
                ...
                if pr:
                    ...
                    for pt in obj.pts:
                        shadow = max(0.1, min(1.0, (pt[0]*-obj.pos[0] + pt[2]*-obj.pos[2])/(obj.r*obj.r)+1.2)) if obj.name != "SUN" else 1.0
                        pr_pt = self.gfx.project([pt[i]+obj.pos[i] for i in range(3)], self.cam, sy, cy, sp, cp, fov, shk)
        ...
        for obj, pr, d, is_t in hud:
            obj.t_tex.alpha = int(obj.hud_alpha * 255); self.rend.draw_color = (*obj.color, int(obj.hud_alpha * 200))
            ...
            self.rend.blit(obj.t_tex, pygame.Rect(int(pr[0]-obj.t_size[0]//2), int(pr[1]-int(oy)), obj.t_size[0], obj.t_size[1]))
            ds = FONT.render(f"{int(d)} KM", True, (220, 220, 220))
            dtex = Texture.from_surface(self.rend, ds)
            dtex.alpha = int(obj.hud_alpha * 180)
            self.rend.blit(dtex, pygame.Rect(int(pr[0]-ds.get_width()//2), int(pr[1]-int(oy)+22), ds.get_width(), ds.get_height()))
